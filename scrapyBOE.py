...
class BOESpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        #print('Si esto es iteeeeem: ' + item)
        ...
        if(firmas == []):
            ...
            for i, elem in enumerate(firmas):
                ...
                with open('nombres-2015.csv', encoding='utf8') as f:
                    reader = csv.reader(f)
                    for row in reader:
                        #print('localocalizacion canabis de calidad y baratoo')
                        #print("nombre: {0}, cantidad: {1}, anio: {2}".format(row[0], row[1], row[2]))
                        separadito = row[0].split()
                        if(separadito[0].lower() == texto[0].lower() and encontrado == False):
                            print()
                            encontrado = True
                            print('El nombre: ' + separadito[0] + '  y el texto: ' + texto[0] + '-------------------------------------------------------------------------------------------------------------')
...
def buscar(posicion, palabra, locali):
    if(posicion != -1):
        #print('la palabra es: ' + palabra.lower())
        for i in range(len(localidades[posicion])):
            #print(localidades[posicion][i].lower())
